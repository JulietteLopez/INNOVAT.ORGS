...
def ed_tipo_nom(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from nomina where id_nomina = {0}'.format (id))
...
def modifica_tipo_nomina(id):
    if request.method == 'POST':
        ...
        if (descr[0] != 0):
            ...
            cursor.execute('update tipo_nomina set  descripcion=%s where id_nomina=%s', (aux_descripcion, id))
            conn.commit()
...
def periodo():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select idperiodo, descripcion, n_dias from periodo order by descripcion')
    datos=cursor.fetchall()
    conn.close()
...
def periodo_agr():
    if request.method == 'POST':
        ...
        if (niveles[0] != 0):
            ...
            cursor.execute('insert into periodo (Descripcion, n_dias) values (%s,%s)', (aux_descripcion, aux_ndias))
            conn.commit()
            conn.close()
...
def ed_periodo(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from nomina where idperiodo= {0}'.format (id))
    ...
    if (peri[0] != 0):
        ...
        return render_template("advertencia.html", des_error=error, paginaant="/periodo")
         ...
         cursor.execute('select idperiodo, descripcion, n_dias from periodo where idperiodo= %s', (id))
         dato=cursor.fetchone()
         conn.close()
...
def modifica_periodo(id):
    if request.method == 'POST':
        ...
        if (per[0] != 0):
            ...
            cursor.execute('update periodo set descripcion=%s, n_dias=%s where idperiodo=%s', (aux_descripcion, aux_ndias, id))
            conn.commit()
            conn.close()
            return redirect(url_for('periodo'))
...
def percepciones():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_Percepcion, Descripcion, Grabado  from percepcion order by descripcion')
    datos=cursor.fetchall()
    conn.close()
...
def agrega_percepcion():
    if request.method == 'POST':
        ...
        if (percepcion[0] != 0):
            ...
            cursor.execute('insert into percepcion (Descripcion, Grabado) values (%s,%s)', (aux_descripcion, aux_grabado))
            conn.commit()
            conn.close()
...
def ed_percepciones(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from percepcion where id_Percepcion= {0}'.format (id))
    '''peri = cursor.fetchone()
    if (peri[0] != 0):
        error = "El periodo de pago tiene dependientes, no puede ser editada."
        return render_template("advertencia.html", des_error=error, paginaant="/periodo")
    else:'''
    cursor.execute('select id_Percepcion, Descripcion, Grabado from percepcion where id_Percepcion= %s', (id))
    dato=cursor.fetchone()
    conn.close()
...
def bo_percepciones(id):
    ...
    cursor.execute('select count(*) from percepcion where id_Percepcion = {0}'.format(id))
    #percepcion = cursor.fetchone()
    cursor.execute('delete from percepcion where id_Percepcion = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('percepciones'))
...
def cal_isr():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_isr, limite_inf, limite_sup, cuota_fija, porcentaje from tabla_isr order by limite_inf')
    datos=cursor.fetchall()
    conn.close()
...
def agrega_isr():
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('Select count(*) from tabla_isr where (limite_sup <= %s and limite_inf >= %s) or (limite_inf <= %s and limite_sup >= %s)', (aux_limsup, aux_liminf, aux_limsup, aux_liminf))
        incorrecto = cursor.fetchone()
        print(incorrecto)
        if (lim_in>=lim_sup or porc>100 or porc<0 or lim_in<0 or lim_sup<0 or cuo<0):
            error = "No puede haber números negativos, números intermedios, números iguales y/o El porcentaje no puede ser mayor a 100% "
        ...
        if (incorrecto[0] != 0):
            ...
            cursor.execute('insert into tabla_isr (limite_inf, limite_sup, cuota_fija, porcentaje) values (%s,%s,%s,%s)', (aux_liminf, aux_limsup, aux_cuofi, aux_por))
            conn.commit()
            conn.close()  
...
def ed_isr(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from tabla_isr where id_isr = {0}'.format (id))
    '''eisr = cursor.fetchone()
    if (eisr[0] != 0):
        error = "El tipo de nomina tiene dependientes, no puede ser editada."
        return render_template("advertencia.html", des_error=error, paginaant="/cal_isr")
    else:'''
    cursor.execute('select id_isr, limite_inf, limite_sup, cuota_fija, porcentaje from tabla_isr where id_isr = %s', (id))
    dato=cursor.fetchall()
    conn.close()
...
def obrero():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_base, descripcion, calculo  from base_imss order by descripcion')
    datos=cursor.fetchall()
    conn.close()
...
def agrega_base():
    if request.method == 'POST':
        ...
        if (obrero[0] != 0):
            ...
            cursor.execute('insert into base_imss (descripcion, calculo) values (%s,%s)', (aux_descripcion, aux_calculo))
            conn.commit()
            conn.close()
...
def ed_obrero(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from tabla_imss where id_base= {0}'.format (id))
    obrero= cursor.fetchone()
    if (obrero[0] != 0):
        ...
        cursor.execute('select id_base, descripcion, calculo from base_imss where id_base= %s', (id))
        dato=cursor.fetchone()
        conn.close()
...
def cuota_agr():
    if request.method == 'POST':
        ...
        if (datos1[0] != 0):
            ...
            cursor.execute('insert into tabla_imss (Descripcion, Porcentaje, id_base) values (%s, %s, %s)',(aux_Descripcion, aux_Porcentaje, aux_base))
            cursor.execute('select a.id_Concepto, a.Descripcion, a.Porcentaje, a.id_base, b.descripcion'
            ' from tabla_imss a, base_imss b '
            ' where b.id_base=a.id_base')

            conn.commit()
            cursor.execute('select descripcion from base_imss order by Descripcion')
            datos = cursor.fetchall()
            conn.close()
...
def ed_cuota(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_Concepto, Descripcion, Porcentaje, id_base from tabla_imss where id_Concepto=%s', (id))
    datos = cursor.fetchone()
    cursor.execute('select id_base, Descripcion from base_imss order by Descripcion')
    datos1 = cursor.fetchall()
...
def deduccion_agr():
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from deducciones where Descripcion = %s',(aux_descripcion))
        deducciones = cursor.fetchone()
        if (deducciones[0] != 0):
            error = "La deduccion ya se encuentra agregado."
            return render_template("error.html", des_error=error, paginaant="/deduccion")
        else:
            cursor.execute('insert into deducciones (Descripcion) values (%s)', (aux_descripcion))
            conn.commit()
            conn.close()
...
def ed_deduccion(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from deducciones where Id_deduccion= {0}'.format (id))
    ...
    cursor.execute('select Id_deduccion, descripcion from deducciones where Id_deduccion= %s', (id))
    dato=cursor.fetchone()
    conn.close()
    return render_template("edi_deduccion.html", deducciones = dato)
...
def modifica_deduccion(id):
    if request.method == 'POST':
        ...
        if (deduccion[0] != 0):
            ...
            cursor.execute('update deducciones set  descripcion=%s where Id_deduccion=%s', (aux_descripcion, id))
            conn.commit()
            conn.close()
            return redirect(url_for('deduccion'))
...
def bo_deduccion(id):
    ...
    cursor.execute('delete from deducciones where Id_deduccion = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('deduccion'))
...
def nomina():
    ...
    datos = cursor.fetchall()
    conn.close()
    return render_template("tabla_nomina.html", nominas = datos)
...
def buscar_nomina():
    if request.method == 'POST':
	   ...
	   ' from nomina a, tipo_nomina b, periodo c, area d '
	   ' where b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and anio=%s ', (busca))
        datos=cursor.fetchall()
        conn.close()
        return render_template("tabla_nomina.html", nominas = datos)
...
def buscar_area():
    if request.method == 'POST':
        busca_area = request.form['bu_area']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute(' select a.id_calnomina, a.id_nomina, b.Descripcion, a.idperiodo, c.Descripcion, a.idArea, d.AreaNombre, a.fecha_nomina, a.anio, a.con_nomina '
	   ' from nomina a, tipo_nomina b, periodo c, area d '
	   ' where b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and AreaNombre=%s ', (busca_area))
        datos=cursor.fetchall()
        conn.close()
        return render_template("tabla_nomina.html", nominas = datos)
...
def agrega_nomina():
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from nomina where id_calnomina = %s',(aux_id))
        nom = cursor.fetchone()
        if (nom[0] != 0):
            error = "La nómina ya se encuentra agregada."
            return render_template("error.html", des_error=error, paginaant="/nueva_nomina")
        else:
            #cursor.execute('insert into nomina (id_nomina, idperiodo, idArea, con_nomina, anio, fecha_nomina) values (%s, %s, %s, %s, %s, %s)', (aux_tipo_nom, aux_periodo, aux_area, aux_consec, aux_anio, aux_fecha))
            cursor.execute('insert into nomina (id_nomina, idperiodo, idArea, fecha_nomina, anio, con_nomina) values (%s, %s, %s, %s, %s, %s)', (aux_tipo_nom, aux_periodo, aux_area, aux_fecha, aux_anio, aux_consec))
            conn.commit()
...
def ed_nomina(id):
    ...
    cursor.execute('select a.id_calnomina, a.id_nomina, b.Descripcion, a.idperiodo, c.Descripcion, a.idArea, d.AreaNombre, a.fecha_nomina, a.anio, a.con_nomina, e.id_calnomina  '
	                ' from nomina a, tipo_nomina b, periodo c, area d, detalle_nomina e'
	                ' where b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and e.id_calnomina=a.id_calnomina ')
    datos1=cursor.fetchall()
...
def bo_nomina(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    '''cursor.execute('select count(*) from nomina where idPuesto = {0}'.format(id))
    solicitudes = cursor.fetchone()
    if (solicitudes[0] != 0):
        error = "El puesto tiene dependientes, no puede ser borrado."
        return render_template("error.html", des_error=error, paginaant="/puesto")
...
def nueva_det_nomina(id):
    ...
    conn.close()
    return render_template("agr_det_nomina.html", det_nominas = datos2)
...
def agrega_det_nomina(id):
    if request.method == 'POST':
        ...
        print("-----------------------------------------------------------------------")
        print(cursor)
        cursor.execute('select count(*) from detalle_nomina where Curp = %s',(aux_curp))
        det = cursor.fetchone()
        print("-----------------------------------------------------------------------")
        print(det)
        if (det[0] != 0):
            ...
            cursor.execute('insert into detalle_nomina (Curp, salario_base, tot_per, isr, imss, tot_ded, salario_neto, tot_grabado) values (%s, %s, %s, %s, %s, %s, %s, %s)', 
                            (aux_curp, aux_base, aux_tot_per, aux_isr, aux_imss, aux_tot_ded, aux_neto, aux_grabado))
            conn.commit()
            print("-----------------------------------------------------------------------")
            print(cursor)
            ('select a.Id_calnomina, a.Curp, a.salario_base, a.tot_per, a.isr, a.imss, a.tot_ded, a.salario_neto, a.tot_grabado, b.Nombre, c.Salario '
                             'from detalle_nomina a, empleado b, contrato c'
                             'where b.Curp = a.Curp and c.Curp = b.Curp and c.Curp = a.Curp')
            datos1=cursor.fetchall()
            cursor.execute('select Curp, Nombre from empleado')
            datos2 = cursor.fetchall()

            cursor.execute('select Curp, Salario from contrato ')
            datos3 = cursor.fetchall()
            conn.close()
    return render_template("edi_nomina.html", nomina= datos1, empleado = datos2, contrato = datos3)

@app.route('/ed_det_nomina/<string:id>')
def ed_det_nomina(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_calnomina, Curp, Salario_base, tot_per, isr, imss, tot_ded, salario_neto, tot_grabado '
                   ' from detalle_nomina where id_calnomina=%s', (id))   
    datos = cursor.fetchone()
    
    cursor.execute('select a.id_calnomina, a.Curp, b.Nombre, c.Salario, a.salario_base, a.tot_per, a.isr, a.imss, a.tot_ded, a.salario_neto, a.tot_grabado '
                    'from detalle_nomina a, empleado b, contrato c '
                    'where b.Curp = a.Curp and c.Curp = b.Curp and c.Curp = a.Curp;')
    datos1=cursor.fetchall()
    cursor.execute('select Curp, Nombre from empleado')
    ...
    datos3 = cursor.fetchall()
    conn.close()
    return render_template("edi_det_nomina.html", detalle = datos, det_nomina = datos1, empleado = datos2, contrato = datos3)

@app.route('/modifica_det_nomina/<string:id>', methods=['POST'])
def modifica_det_nomina(id):
    if request.method == 'POST':
        aux_curp = request.form['curp']
        aux_base = request.form['sal_base']
        aux_tot_per = request.form['tot_per']
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('update detalle_nomina set Curp=%s, Salario_base=%s, tot_per=%s, isr=%s, imss=%s, tot_ded=%s, salario_neto=%s, tot_grabado=%s '
                        'where id_calnomina=%s', (aux_curp, aux_base, aux_tot_per, aux_isr, aux_imss, aux_tot_ded, aux_neto, aux_grabado, id))
        conn.commit()
        conn.close()
    return redirect(url_for('ed_nomina'))
...
def bo_det_nomina(id):
    ...
    cursor.execute('delete from detalle_nomina where id_calnomina = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('ed_nomina'))
...
#percepcion
@app.route('/agrega_per_nomina/<string:id>', methods=['POST'])
def agrega_per_nomina(id):
    if request.method == 'POST':
        aux_nomina = id
        aux_curp = request.form ['curp']
        aux_percepcion = request.form ['percepcion']
        aux_monto = request.form ['monto']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from per_nomina where id_calnomina = %s and id_Percepcion=%s',(aux_nomina, aux_percepcion))
        existe_pernom = cursor.fetchone()
        if (existe_pernom[0] != 0):
            error = "Esta percepción de nómina ya se encuentra agregada."
            return render_template("error.html", des_error=error, paginaant="/edi_det_nomina")
        else:
            cursor.execute('insert into per_nomina (Curp, id_Percepcion, monto) '
                           ' values (%s,%s,%s,%s)',(aux_curp, aux_percepcion, aux_monto))
            conn.commit()
            cursor.execute('select id_calnomina, id_nomina, idperiodo, idArea, fecha_nomina, anio, con_nomina '
                           ' from nomina where id_calnomina=%s', (aux_nomina))
            datos = cursor.fetchone() 
            print("___________")
            print(datos)
            cursor.execute('SELECT a.id_calnomina, a.id_nomina, a.idperiodo, a.idArea, a.fecha_nomina, a.anio, a.con_nomina, b.descripcion, c.descripcion, d.AreaNombre, e.id_calnomina, f.descripcion, f.Grabado, g.id_Percepcion'
                            'FROM nomina a, tipo_nomina b, periodo c, area d, detalle_nomina e, percepcion f, per_nomina g'
                            'WHERE b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and e.id_calnomina=a.id_calnomina and f.Id_Percepcion=g.Id_Percepcion and a.id_calnomina=%s', (aux_nomina))
            datos1 = cursor.fetchall()
            print("___________")
            print(datos1)           
            cursor.execute('select id_calomina, con_nomina from nomina')
            datos2 = cursor.fetchall()
            ...
            datos4 = cursor.fetchall()
            conn.close()
            return render_template("edi_det_nomina.html", nomina = datos, per_nomina = datos1, cal_nomina = datos2, percepcion = datos3, curp = datos4)


@app.route('/bor_per_nomina/<string:idPE>/<string:idN>')
def bor_per_puesto(idPE,idN):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('delete from per_nomina where id_Percepcion =%s and id_calnomina =%s',(idPE,idN))
    conn.commit()
    cursor.execute('select id_calnomina, id_nomina, idperiodo, idArea, fecha_nomina, anio, con_nomina '
    ...
    cursor.execute('SELECT a.id_calnomina, a.id_nomina, a.idperiodo, a.idArea, a.fecha_nomina, a.anio, a.con_nomina, b.descripcion, c.descripcion, d.AreaNombre, e.id_calnomina, f.descripcion, f.Grabado, g.id_Percepcion'
                            'FROM nomina a, tipo_nomina b, periodo c, area d, detalle_nomina e, percepcion f, per_nomina g'
                            'WHERE b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and e.id_calnomina=a.id_calnomina and f.Id_Percepcion=g.Id_Percepcion and a.id_calnomina=%s', (idN))
    datos1 = cursor.fetchall()
    cursor.execute('select id_calomina, con_nomina from nomina')
    datos2 = cursor.fetchall()
    cursor.execute('select id_Percepcion, Descripcion from percepcion order by Descripcion')
...
#deduccion
@app.route('/agrega_ded_nomina/<string:id>', methods=['POST'])
def agrega_ded_nomina(id):
    if request.method == 'POST':
        # aux_nomina = id
        # aux_curp = request.form ['curp']
        aux_deduccion = request.form ['deduccion']
        aux_monto = request.form ['monto']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from ded_nomina where id_calnomina = %s and id_deduccion=%s',(aux_deduccion))
        existe_dednom = cursor.fetchone()
        if (existe_dednom[0] != 0):
            error = "Esta deducción de nómina ya se encuentra agregada."
            return render_template("error.html", des_error=error, paginaant="/edi_det_nomina")
        else:
            cursor.execute('insert into per_nomina ( id_deduccion, monto) '
                           ' values (%s,%s)',(aux_deduccion, aux_monto))
            conn.commit()
            cursor.execute('select id_calnomina, id_nomina, idperiodo, idArea, fecha_nomina, anio, con_nomina '
                           ' from nomina where id_deduccion=%s', (aux_deduccion))
            datos = cursor.fetchone() 
            print("___________")
            print(datos)
            cursor.execute('SELECT a.id_calnomina, a.id_nomina, a.idperiodo, a.idArea, a.fecha_nomina, a.anio, a.con_nomina, b.descripcion, c.descripcion, d.AreaNombre, e.id_calnomina, f.descripcion, f.Grabado, g.id_deduccion'
                            'FROM nomina a, tipo_nomina b, periodo c, area d, detalle_nomina e, deducciones f, ded_nomina g'
                            'WHERE b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and e.id_calnomina=a.id_calnomina and f.Id_deduccion=g.Id_deducciones and a.id_deduccion=%s', (aux_deduccion))
            datos1 = cursor.fetchall()
            print("___________")
            print(datos1)           
            cursor.execute('select id_calomina, con_nomina from nomina')
            ...
            datos4 = cursor.fetchall()
            conn.close()
            return render_template("edi_det_nomina.html", nomina = datos, ded_nomina = datos1, cal_nomina = datos2, deducciones = datos3, curp = datos4)


@app.route('/bor_ded_nomina/<string:idDE>')
def bor_ded_nomina(idDE):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('delete from per_nomina where id_deduccion =%s',(idDE))
    conn.commit()
    cursor.execute('select id_calnomina, id_nomina, idperiodo, idArea, fecha_nomina, anio, con_nomina '
                           ' from nomina where id_calnomina=%s', (idDE))
    datos = cursor.fetchone()            
    cursor.execute('SELECT a.id_calnomina, a.id_nomina, a.idperiodo, a.idArea, a.fecha_nomina, a.anio, a.con_nomina, b.descripcion, c.descripcion, d.AreaNombre, e.id_calnomina, f.descripcion, f.Grabado, g.id_deduccion'
                            'FROM nomina a, tipo_nomina b, periodo c, area d, detalle_nomina e, deducciones f, ded_nomina g'
                            'WHERE b.id_nomina=a.id_nomina and c.idperiodo=a.idperiodo and d.idArea=a.idArea and e.id_calnomina=a.id_calnomina and f.Id_deduccion=g.Id_deducciones and a.id_calnomina=%s', (idDE))
    datos1 = cursor.fetchall()
    cursor.execute('select id_calomina, con_nomina from nomina')
    datos2 = cursor.fetchall()
    cursor.execute('select id_deduccion, Descripcion from deducciones order by Descripcion')
